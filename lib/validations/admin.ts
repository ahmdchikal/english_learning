...
export const levelFormSchema = z.object({
  ...
  force_unlocked: z.boolean().default(false),
...
export const lessonFormSchema = z.object({
  ...
  grammar_notes: z.string().trim().max(5000).default(""),
  common_mistakes: z.string().trim().max(5000).default(""),
...
export const vocabularyFormSchema = z.object({
  ...
  phonetic: z.string().trim().max(100).default(""),
  example_sentence: z.string().trim().max(500).default(""),
  example_translation: z.string().trim().max(500).default(""),
  word_type: z.string().trim().max(50).default("noun"),
  order_index: z.number().int().min(0),
});
...
export const questionFormSchema = z.object({
  ...
  is_placement_question: z.boolean().default(false),
  ...
  audio_text: z.string().trim().max(500).nullable().optional(),
